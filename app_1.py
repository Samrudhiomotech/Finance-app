...
def determine_user_profile(input_data, savings_amount, resilience_score):
    ...
    profile = {
        ...
        'savings_behavior': 'good' if savings_amount / input_data['Income'] > 0.15 else 'moderate' if savings_amount / input_data['Income'] > 0.05 else 'poor',
...
def calculate_estimated_benefit(scheme_id, input_data, savings_amount):
    ...
    if scheme_id == 'PMJDY':
        benefits = {
            'overdraft_limit': min(10000, input_data['Income'] * 0.5),
            'insurance_cover': 130000,
            'annual_cost': 0
        }
    elif scheme_id == 'APY':
        monthly_contribution = min(input_data['Income'] * 0.05, 1000)  # 5% of income or ₹1000, whichever is less
